...
- **Role-aware onboarding** — a first-run tour with member and admin-only
  ...
  that persists across devices. **Per-page guided tours** (spotlight +
  animated tips) on Groups, Overview, Transactions, Forecast, Payments,
  Data Source, and Settings — including an **onboarding mode** that loads
  sample data so empty ledgers still show real UI
