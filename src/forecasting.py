...
# --------------------------------------------------------------------------
# Cache-aware entry points -- these are what app.py should call. They wrap
# the raw fit_* functions above with a DB-backed cache keyed on a hash of
# the member's series, so switching between /forecast and /accuracy (or
# just reloading either) re-fits nothing until the underlying data
# actually changes. This is what keeps a single 512MB Render worker from
# re-running Holt-Winters for every member on every page hit.
# --------------------------------------------------------------------------
...
def get_or_backtest_metrics(group_id: int, member_id: str, series: pd.Series, holdout: int = 3):
    ...
    if len(cached) == 1:  # Holt-Winters row present and fresh
