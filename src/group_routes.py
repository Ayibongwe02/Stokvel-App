...
def preview_sample():
    ...
    if existing is not None:
        ...
        session["active_group_id"] = group.id
        flash(f"Reopened onboarding mode for '{group.name}' with fresh sample data.", "success")
        return redirect(url_for("overview"))
    ...
    session["active_group_id"] = group.id
    flash(
        "Onboarding mode is ready — each page has a short guided tour. "
        "Explore Overview, Forecasts, and Payments with sample data, then create a real group when you’re ready.",
        "success",
    )
    return redirect(url_for("overview"))
...
def exit_sample_preview():
    ...
    if membership is None or not is_sample_preview_group(membership.group):
        flash("You're not in onboarding mode right now.", "error")
        return redirect(url_for("groups.index"))
