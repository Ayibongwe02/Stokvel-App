...
def create_group(client, name="My Stokvel", region="Gauteng", seed=True):
    """Create a group. By default also load sample data so dashboard tests
    still see balances/forecasts. Pass seed=False for an empty real group
    (create no longer auto-seeds)."""
    resp = client.post("/groups/create", data={"name": name, "region": region}, follow_redirects=True)
    if seed:
        client.post("/data/reset", follow_redirects=True)
    return resp


def preview_sample(client):
    return client.post("/groups/preview-sample", follow_redirects=True)
...
def test_signup_then_redirected_to_groups(client):
    ...
    # Fresh accounts land on the get-started hub, not the ledger.
    assert b"How do you want to begin" in resp.data or b"Create a group" in resp.data
    assert b"Preview with sample data" in resp.data
...
def test_create_group_starts_empty_without_sample(client):
    signup(client)
    create_group(client, name="Clean Group", seed=False)
    resp = client.get("/")
    assert resp.status_code == 200
    assert b"No data yet" in resp.data
    assert b"Group Overview" not in resp.data


def test_preview_sample_loads_overview_with_data(client):
    signup(client)
    resp = preview_sample(client)
    assert resp.status_code == 200
    assert b"Group Overview" in resp.data or b"Total Group Balance" in resp.data
    assert b"Sample Preview" in resp.data or b"sample" in resp.data.lower()


def test_get_started_hub_shows_action_cards(client):
    signup(client)
    resp = client.get("/groups/")
    assert resp.status_code == 200
    assert b"Create a group" in resp.data
    assert b"Preview with sample data" in resp.data
    assert b"Add a member" in resp.data
    assert b"Join with an invite code" in resp.data
...
def test_exit_sample_preview_returns_to_hub(client):
    signup(client)
    preview_sample(client)
    resp = client.post("/groups/exit-sample-preview", follow_redirects=True)
    assert resp.status_code == 200
    assert b"How do you want to begin" in resp.data or b"Create a group" in resp.data
    assert b"Exit sample preview" not in resp.data or b"Preview with sample data" in resp.data
    # Floating exit control should not appear once left
    assert b"exit-sample-widget" not in resp.data
